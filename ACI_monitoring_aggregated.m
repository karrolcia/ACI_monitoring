%% Processing data with ACI monitoring scheme - aggregated data
close all; clear all;
...
time = agg_cloudnet.time2;
height = agg_cloudnet.height;


%% Array dimensons
n_time = size(agg_cloudnet.time2,2);
n_height = size(agg_cloudnet.height,2);
height_res = (agg_cloudnet.height(2)-agg_cloudnet.height(1));
...
% below the cloud
[Peak_P, I_max_agg] = max(agg_cloudnet.beta, [], 2) ;
I_max_agg(isnan(I_max_agg))=1;
low_cloud = (find(height > 2000, 1, 'first'));
filter_beta = find(I_max_agg(I_max_agg ==1));
...
for ih = 1:n_time
    if I_max_agg(ih) < 16 
        for jj=1:numel(var_to_nan)
            var = var_to_nan{jj};
            agg_cloudnet.(var)(ih)=NaN;
        end
        for ji = 1:numel(var_to_nan2)
            var2 = var_to_nan2{ji};
            agg_retrival.(var2)(ih)=NaN;
        end
    end
    if I_max_agg(ih) > low_cloud  
        for jj=1:numel(var_to_nan)
            var = var_to_nan{jj};
            agg_cloudnet.(var)(ih)=NaN;
        end
...
I_max_agg(I_max_agg < 16)=NaN;
I_max_agg(I_max_agg > low_cloud)=NaN;
% Filtering the data based on the Cloudnet catgory bits
bits=agg_cloudnet.category_bits;
...
z_max=-20; %dBZ threshold for drizzle (see e.g. Frisch et al. 2000, Matrosov, 2004)
z_min=-60;
lwp_min=15;%g/m^2 threshold for MWR-LWP retrieval
lwp_max=200;
water_data=bit0;
% only use cases when LWP is larger than 15g/m^2
water_data(agg_cloudnet.lwp<lwp_min,:)=0;
water_data(agg_cloudnet.lwp>lwp_max,:)=0;
% selection criteria: no drizzle and single layer

%% Declare parameters used for filtering 
Z_water=NaN(size(agg_cloudnet.Z(:,:)));    %single layer water cloud Z-profile for retrieval input
...
for i=1:n_time;
     ...
     Z_water(i,is_water)=agg_cloudnet.Z(i,is_water);
...
beta_sel = NaN(size(agg_cloudnet.beta(:,:)));
beta_sel(is_refl,:) = agg_cloudnet.beta(is_refl,:);
temp_beta = beta_sel;
...
int_beta_atten(int_beta_atten<=1.0e-4) = NaN;
int_beta_atten(int_beta_atten>0.005) = NaN;
clear temp_beta
Azores_plots_agg
